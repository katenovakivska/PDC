...
public class Process extends Element
{
        ...
        public Process(double delay, String name, String distribution, int maxQueueLength, int maxParallel, double devDelay, double timeOfModeling)
        {
            ...
            TimeOfModeling = timeOfModeling;
            for (int i = 0; i < MaxParallel; i++)
            {
                String str = Name+" -> Channel"+(i + 1);
                Channels.add(new Channel(str, 0.0, true, this));
            }
        }
        public Process(double delay, String name, String distribution, int maxQueueLength, int maxParallel, double timeOfModeling)
        {
        ...
        public List<Channel> OutChannel(double t)
        {
            ...
            for (Channel c: Channels)
            {
                if (Channels.size() != 0)
                {
                    ...
                    {
                        if(!c.Seize.IsRunning)
                        c.Executor.execute(c.Release);
                        c.IsFree = true;
                        outChannels.add(c);
                    }
                }
        ...
        public void InChannel()
        {
            ...
            if (NumberOfTasks <= numberOfFreeDevices && NumberOfTasks > 0)
            {
                for (int i = 0; i < Channels.size(); i++)
                {
                    ...
                    {
                        if(!Channels.get(i).Seize.IsRunning)
                        Channels.get(i).Executor.execute(Channels.get(i).Seize);
                        Channels.get(i).Delay = GetDelay();
                        Channels.get(i).TimeOut = TCurrent + Channels.get(i).Delay;
